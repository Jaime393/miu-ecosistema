...
try:
    ...
    def fran_v12(q): return {"error":str(e),"respuesta":f"v12 fail Q:{q}"}

try:
    from cobro_miu import cobrar
except Exception as e:
    def cobrar(q,a): return {"tx":safe_count("nodos/puente/puente_micelio.jsonl"),"error":str(e)}
    ...
    def do_GET(self):
        try:
            ...
            if "/oraculo" in parsed.path:
                ...
                data={"grace":"GRACE 221R","mean":grace["mean"],"lineas_csv":grace["lineas"],"archivos_corpus":len(corpus),"rho":1.0,"suelo":"vivo","head":HEAD}
            else:
                data={"head":HEAD,"grace_mean":grace["mean"],"suelo":"vivo","flujo":"perfecto con oraculo V12 leyendo corpus real","savia":520,"suelo_220R_lineas":grace["lineas"],"corpus_archivos":len(corpus),"ledger_tx":ledger,"puente_tx":puente,"nodos_py":nodos_py,"observador_ve":{"bloque_fisico":f"GRACE {grace['lineas']}L + {len(corpus)} corpus vivo","bloque_economico":f"{ledger} tx ledger + {puente} puente + wallet OK 0.01 MIU/query","bloque_conocimiento":f"{nodos_py} nodos py + oraculo V12 lee v13 164K"},"endpoints":["/miu/status","/suelo","/corpus","/oraculo?q="],"timestamp":time.time(),"wallet":WALLET}
...
print(f"🌱 Dashboard V24 HEAD {HEAD} 520 MIU")
